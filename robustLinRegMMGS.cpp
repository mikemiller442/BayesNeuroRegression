...
public:
  ...
  arma::Mat<double> priorMat,covMat;
  arma::Mat<double> XTX;
  arma::Col<double> XTy;
  arma::Col<double> Y;
  arma::Col<double> cotangentVec;
...
// contrast in a regression model that potentially uses multiple task contrasts.
...
public:
  ...
  arma::Mat<double> U;
  arma::Col<double> D;
  arma::Mat<double> V,tV,Z,Zinv;
  arma::Col<double> Zdiag,ZinvDiag;
  arma::Col<double> cotangentVec;
  ...
  void construct(arma::Mat<double> X, arma::Mat<double> testX,
    ...
    Zinv = arma::eye(X.n_cols, X.n_cols);
    ZinvDiag = arma::vec(X.n_cols);
...
// effects models. Only one task contrast can be used in this regression model.
class LinRegModelGS {
public:
  arma::Col<double> Y,testY,regVarScale,regVar,XTy;
  arma::Mat<double> B,S,features,XTX;
  int n,testN,numEpochs;
  double regVarScaleAlpha,regVarScaleBeta,regVarAlpha,regVarBeta,regVarPrior;
  arma::Col<double> XY1A,XY1B,XY2A,XY2B,XY2C,B1A,B1B,B2A,B2B,B2C;
  ...
  void construct(arma::Mat<double> Z, arma::Mat<double> testZ,
    ...
    B = arma::trans(hp.features)*me.features;
  ...
  void sampleVarParams(int epoch) {
    ...
    for (int i = 0; i < numGroups; i++) {
      wishB = wishB + me.beta.submat(numFE+i*numRE,epoch+1,
                                     numFE+(i+1)*numRE-1,epoch+1)
      *arma::trans(me.beta.submat(numFE+i*numRE,epoch+1,
                                  numFE+(i+1)*numRE-1,epoch+1));
    }
...
Rcpp::List linRegMMGS(NumericMatrix Z, NumericMatrix testZ,
  ...
  for (int epoch = 0; epoch < numEpochs; epoch++) {
    ...
    mod.me.invPriorMat = arma::zeros(mod.me.numWeights,
                                     mod.me.numWeights);
    mod.me.invPriorMat(0,0) = (1.0/mod.regVar(epoch))*(1.0/mod.me.interceptPrior);
    mod.me.invPriorMat.submat(1,1,mod.numFE-1,
                              mod.numFE-1) = arma::eye(mod.numFE-1,mod.numFE-1)
      *(1.0/mod.regVar(epoch))*(1.0/mod.me.betaPrior);
    mod.me.invPriorMat.submat(mod.numFE,mod.numFE,
                              mod.numFE + mod.numGroups*mod.numRE-1,
                              mod.numFE + mod.numGroups*mod.numRE-1) = arma::kron(arma::eye(mod.numGroups,mod.numGroups),
                                                                                  arma::inv(mod.Sigma[epoch]));
    A.submat(0,0,mod.me.numWeights-1,mod.me.numWeights-1) = A.submat(0,0,mod.me.numWeights-1,
             mod.me.numWeights-1)
      + mod.me.invPriorMat;
    A.submat(mod.me.numWeights,mod.me.numWeights,mod.me.numWeights+mod.hp.numWeights-1,
             ...
             mod.me.numWeights+mod.hp.numWeights-1)
      + arma::eye(mod.hp.numWeights,mod.hp.numWeights)
      *(1.0/mod.regVar(epoch))*(1.0/mod.hp.lambdaSq(epoch));
    ...
    mod.hp.beta.col(epoch + 1) = beta.subvec(mod.me.numWeights,
                    mod.me.numWeights+mod.hp.numWeights-1);
